...
public class Es_233_Facets {
    ...
    private static void histogramFacet() {
        ...
        for (HistogramFacet.Entry entry : histogramFacet) {
            System.out.println("entry.getKey()->" + entry.getKey() + "\t entry.getCount()->" + entry.getTotal());
        }
    }
    ...
    private static Map searchByQuery_Facets(FacetBuilder facetBuilder) {
        ...
        final String INDEX = "ezsonar_*";
        final String TYPE = "message";
        SearchResponse response = Es_233_Utils.client.prepareSearch(INDEX)
                .setTypes(TYPE)
               // .setQuery(QueryBuilders.inQuery("streams","525e661a84ae03095e71d758"))
